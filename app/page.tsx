...
export default function Home() {
  ...
  useEffect(() => {
    const a = new URLSearchParams(window.location.search).get('address') as `0x${string}` | null;
    if (a) setAddress(a);
  }, []);
  ...
  useEffect(() => {
    if (!address) return;
    let active = true;
    publicClientFor(chain).getBalance({ address })
      .then((v) => active && setBalance(formatEther(v)))
  ...
  async function unlockWallet() {
    try {
      setError('');
      const phrase = await decryptWallet(unlock);
      setAddress(accountFromMnemonic(phrase).address);
      setUnlock('');
    } catch {
      setError('Unable to unlock wallet. Check your password.');
    }
  }

  if (!address) {
    return <main className="shell"><section className="card hero">
      <p className="eyebrow">AURELIS</p>
      <h1>Your keys.<br/>Your assets.<br/>Your control.</h1>
      <p className="muted">A clean, non-custodial multi-chain wallet.</p>
      <div className="actions"><a className="primary" href="/wallet/create">Create wallet</a><a className="secondary" href="/wallet/import">Import wallet</a></div>
      {hasEncryptedWallet() && <div className="unlock"><label>Unlock existing wallet</label><input type="password" value={unlock} onChange={e => setUnlock(e.target.value)} placeholder="Wallet password"/><button className="primary" onClick={unlockWallet}>Unlock</button>{error && <div className="error">{error}</div>}</div>}
    </section></main>;
  }

  const activityHref = `/transactions?address=${encodeURIComponent(address)}&chainId=${chain.id}`;
  const isArc = chain.id === 5042;
  return <main className="shell"><header className="top"><div><p className="eyebrow">AURELIS</p><h2>Portfolio</h2></div><span className="address">{address.slice(0,6)}…{address.slice(-4)}</span></header>
