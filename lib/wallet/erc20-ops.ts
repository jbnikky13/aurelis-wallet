...
export function normalizeTokenAmount(value: bigint): bigint {
  return assertAmount(value);
}

export function buildErc20Transfer(token: Address, recipient: Address, amount: bigint): Erc20Call {
  return {
    address: assertAddress(token),
    abi: erc20Abi,
    functionName: 'transfer',
    args: [assertAddress(recipient), normalizeTokenAmount(amount)],
  };
}
...
export function buildErc20Approval(token: Address, spender: Address, amount: bigint, allowUnlimited = false): Erc20Call {
  ...
  if (normalized === MAX_ERC20_APPROVAL && !allowUnlimited) {
    throw new Error('Unlimited token approval requires explicit confirmation.');
  }
  return {
    address: assertAddress(token),
    abi: erc20Abi,
    functionName: 'approve',
    args: [assertAddress(spender), normalized],
  };
}
...
export function describeAllowance(allowance: bigint) {
  ...
  if (allowance === MAX_ERC20_APPROVAL) {
    return { state: 'unlimited' as const, warning: 'Unlimited token allowance is active. Consider revoking it when no longer needed.' };
  }
